...
const PRESET_COLORS = [
  '#000000', '#ffffff', '#f97316', '#fde047', '#22c55e', '#38bdf8', '#818cf8', '#ec4899', '#a3e635', '#f8b195', '#c06c84', '#6c5b7b'
];
...
function renderPalette() {
  ...
  colors.forEach(colorCode => {
    ...
    // Allow removing custom colors via right-click
    button.addEventListener('contextmenu', (ev) => {
      ev.preventDefault();
      const idx = customPalette.indexOf(colorCode);
      if (idx !== -1) {
...
function addColorToPalette() {
  ...
  if (!customPalette.includes(newColor)) {
    customPalette.unshift(newColor);
